...
public class DataDividerByUser {
	...
	public static class DataDividerReducer extends Reducer<IntWritable, Text, IntWritable, Text> {
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			//output: key = user, value = movie1:rating, movie2:rating...

			StringBuilder movie_rating = new StringBuilder();
			for (Text value : values) {
				movie_rating.append(value + ",");
			}
			if(movie_rating.length() > 0) {
				movie_rating.deleteCharAt(movie_rating.length() - 1);
			}
			context.write(key, new Text(movie_rating.toString()));
		}
	}
